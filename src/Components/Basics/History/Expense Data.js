...
const ExpenseList = () => {
    ...
    const getEmp = async () => {
        try {
            const res = await axios.get(`${process.env.REACT_APP_BaseURl}/get-expense`);
            if (res) {
                setIncList(res.data)
            }
        } catch (err) {
                ...
                <tbody>
                    {IncList.length && IncList.map((Income, index) => (
                        <tr key={index}>
                            ...
                            <td><Link className="btn btn-link" role="tab" type="button" to={`/get-expense/${Income._id}`}>Edit</Link></td>
                        </tr>
                    ))}
                </tbody>
            </table>
